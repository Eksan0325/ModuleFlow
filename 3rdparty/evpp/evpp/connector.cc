...
void Connector::HandleWrite() {
    ...
    if (chan_->fd() <= 0) {
        LOG_ERROR << "Connector::HandleWrite, fd=" << chan_->fd() << ", addr= " << laddr;
    }
    timer_->Cancel();
    timer_.reset();
    chan_->DisableAllEvent();
...
void Connector::HandleError() {
    ...
    if (!owner_tcp_client_->auto_reconnect() || serrno != 110) {
    ...
    if (EVUTIL_ERR_CONNECT_REFUSED(serrno) || !owner_tcp_client_->auto_reconnect()) {
        LOG_ERROR << "HandleError , errno : " << serrno << " , auto connect: " << owner_tcp_client_->auto_reconnect();
        conn_fn_(-1, "");
    }
